...
-- 2 - Try pulling standard_qty, gloss_qty, and poster_qty from the orders table, and the website and the primary_poc from
    --  the accounts table.
...
SELECT r.name r_name, 
        s.name s_name, a.name a_name
    FROM region r
        JOIN sales_reps s
            ON r.id =s.region_id
        JOIN accounts a
...
-- These are some facts about joins
...
-- NOTE
-- If you have two or more columns in your SELECT that have the same name after the table name such as accounts.name and 
-- sales_reps.name you will need to alias them. Otherwise it will only show one of the columns.
...
JOIN accounts a
ON a.sales_rep_id = s.id
-- WHERE s.name LIKE 'S%' AND r.name LIKE '%Mid%'
...
JOIN accounts a
ON a.sales_rep_id = s.id
-- WHERE a.name LIKE '%K' AND r.name LIKE '%Mid%'
...
total_amt_usd/(total+0.001) unit_price
FROM region r
JOIN sales_reps s
ON s.region_id = r.id
JOIN accounts a
